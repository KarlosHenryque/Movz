...
function valor(form: FormData, nome: string) {
...
export async function cadastrar(form: FormData) {
  ...
  const validacaoSenha = senhaForteSchema.safeParse(senha);
  if (!validacaoSenha.success) {
    redirect(
      `/cadastro?erro=${encodeURIComponent(validacaoSenha.error.issues[0].message)}`,
    );
  }
  const supabase = await criarClienteServidor();
  ...
  const { error } = await supabase.auth.signUp({
    email: valor(form, "email"),
    password: validacaoSenha.data,
    options: {
      data: { nome: valor(form, "nome") },
...
export async function recuperar(form: FormData) {
  const supabase = await criarClienteServidor();
  const origem = process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000";
  await supabase.auth.resetPasswordForEmail(valor(form, "email"), {
